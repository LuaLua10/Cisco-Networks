...
S2 будет настроен в качестве сервера. Коммутаторы S1 и S3 будут настроены как
клиенты. Они будут входить в домен VTP **CCNA** с паролем **cisco**.
...
\*Mar 1 00:03:44.193: %SW_VLAN-6-VTP_DOMAIN_NAME_CHG: VTP domain name changed to
CCNA.
...
1.  Настройте S1 и S3 в качестве клиентов VTP в домене **CCNA** с паролем
    **cisco**. Конфигурации VTP приведены ниже.
...
1.  Проверьте конфигурации VTP, введя команду **show vtp status** на всех
    коммутаторах. Статус VTP для S3 приведен ниже.
...
1.  Настройка динамического протокола транкинга (DTP)

Настройте динамические магистральные каналы между S1 и S2.
----------------------------------------------------------

1.  Введите команду **show interfaces f0/1 switchport** на коммутаторах S1 и S2.
...
1.  В режиме интерфейсной настройки установите динамический магистральный канал
    между S1 и S2. Поскольку по умолчанию устанавливается режим dynamic auto, то
    только одну сторону канала необходимо перевести в режим dynamic desirable.
...
\*Mar 1 0:30:48.102: %LINEPROTO-5-UPDOWN: Line protocol on Interface
FastEthernet0/1, changed state to up

1.  Проверьте магистральный канал между коммутаторами S1 и S2 с помощью команды
...
1.  Между коммутаторами S1 и S3 установите статический магистральный канал
    с помощью команды **switchport mode trunk** в режиме интерфейсной настройки
    для порта F0/3.
...
1.  Проверьте магистрали с помощью команды **show interfaces trunk** на
    коммутаторе S1.
...
1.  Настройте постоянную магистраль между коммутаторами S2 и S3.

2.  Запишите команды, использованные для создания статической магистрали.
...
1.  Добавление сетей VLAN и назначение портов

Добавьте сети VLAN на коммутаторах.
-----------------------------------

1.  На коммутаторе S1 добавьте сеть VLAN 10.

S1(config)\# **vlan 10**

Удалось ли вам создать сеть VLAN 10 на коммутаторе S1? Поясните ответ.

*Ответ: в режиме VTP клиента нельзя создавать локальную базу VLAN.*

1.  Добавьте следующие сети VLAN на коммутаторе S2.
...
Проверьте обновления VTP на коммутаторах S1 и S3.
-------------------------------------------------

Так как коммутатор S2 настроен как VTP-сервер, а коммутаторы S1 и S3 настроены
как VTP-клиенты, коммутаторы S1 и S3 должны получить и применить информацию
о сети VLAN от коммутатора S2.

>   Какие команды **show** вы использовали для проверки обновлений VTP на
>   коммутаторах S1 и S3?
...
На этом шаге вам предстоит связать порты с сетями VLAN и настроить IP-адреса
...
1.  На коммутаторе S1 переведите порт F0/6 в режим доступа и назначьте его сети
    VLAN 10.
...
1.  Повторите процедуру для порта F0/18 на коммутаторах S2 и S3. Назначьте сеть
    VLAN согласно данной таблице.

2.  Назначьте IP-адреса компьютерам согласно данной таблице.

Настройте IP-адреса на коммутаторах.
------------------------------------

1.  На коммутаторе S1 назначьте IP-адрес интерфейсу SVI для сети VLAN 99
    в соответствии с таблицей адресации и активируйте интерфейс.
...
1.  Повторите шаг a для коммутаторов S2 и S3.

Проверьте наличие сквозного соединения
--------------------------------------

1.  Отправьте ping-запрос с компьютера PC-B на PC-A и проверьте результат.
    Поясните ответ.

*Пинг не проходит, по причине нахождения портов в разных VLAN.*

1.  Отправьте ping-запрос с компьютера PC-A на PC-C и проверьте результат.
    Поясните ответ.

*Пинг проходит, устройства находятся в одном VLAN.*

1.  Отправьте ping-запрос с коммутатора S1 на компьютер PC-A. Была ли проверка
    успешной? Поясните ответ.

*Пинг не проходит, т.к. SVI коммутатора находится в другом VLAN.*

1.  Отправьте ping-запрос с коммутатора S2 на коммутатор S1. Была ли проверка
    успешной? Поясните ответ.

    *Пинг проходит, оба коммутатора находятся в управляющем VLAN.*

2.  Настройка сети VLAN расширенного диапазона

Сеть VLAN расширенного диапазона — это сеть VLAN в диапазоне от 1025 до 4096.
Так как сетями VLAN расширенного диапазона нельзя управлять с помощью VTP,
необходимо перевести VTP в прозрачный режим. В этой части вам предстоит
перевести VTP на коммутаторе S1 в прозрачный режим и создать сеть VLAN
расширенного диапазона на коммутаторе S1.
...
1.  Проверьте режим VTP на коммутаторе S1.
...
Настройте сеть VLAN расширенного диапазона на коммутаторе S1.
-------------------------------------------------------------

1.  Отобразите текущие конфигурации сети VLAN на коммутаторе S1.

2.  Создайте сеть VLAN 2000 расширенного диапазона.
...
1.  Проверьте, что сеть VLAN создана правильно.
